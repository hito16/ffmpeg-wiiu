...
static void video_display(VideoState *is)
{
    ...
    printf("video_display SDL_SetRenderDrawColor/SDL_RenderClear/SDL_RenderPresent\n");
}
...
static int read_thread(void *arg)
{
    ...
    for (i = 0; i < ic->nb_streams; i++)
    {
        ...
        printf("avformat_match_stream_specifier\n");
    ...
    printf("av_find_best_stream video\n");
    if (!video_disable)
        st_index[AVMEDIA_TYPE_VIDEO] =
            av_find_best_stream(ic, AVMEDIA_TYPE_VIDEO,
                                st_index[AVMEDIA_TYPE_VIDEO], -1, NULL, 0);

    printf("av_find_best_stream audio\n");
    if (!audio_disable)
    ...
    printf("av_find_best_stream subtitle\n");
    ...
    printf("av_guess_sample_aspect_ratio and set_default_window_size\n");
    ...
    printf("stream_component_open SUBTITLE\n");
    ...
    if (is->video_stream < 0 && is->audio_stream < 0)
    {
        av_log(NULL, AV_LOG_FATAL, "Failed to open file '%s' or configure filtergraph\n",
               is->filename);
        printf("Failed to open file '%s' or configure filtergraph\n",
               is->filename);
        ret = -1;
        goto fail;
    }
    ...
    printf("Beginning read_thread main loop\n");
    for (;;)
    {
        if (is->abort_request)
            break;
        ...
        {
            packet_queue_put(&is->videoq, pkt);
            strncat(strbuffer, "put video ", 12);
        }
        else if (pkt->stream_index == is->subtitle_stream && pkt_in_play_range)
        {
            packet_queue_put(&is->subtitleq, pkt);
            strncat(strbuffer, "put subs ", 12);
        }
        else
        {
            av_packet_unref(pkt);
                               ...
                               const AVInputFormat *iformat)
{
    ...
    printf("SDL_CreateThread - readthread\n");
...
int main(int argc, char **argv)
{
    ...
    if (window && renderer)
    {
        ...
        SDL_RenderClear(renderer);
        printf("test SDL_RenderClear()\n");
        // Update the screen
        SDL_RenderPresent(renderer);
        printf("test SDL_RenderPresent()\n");
        SDL_DestroyRenderer(renderer);
        SDL_DestroyWindow(window);
        printf("test SDL_RenderPresent()\n");
    }
