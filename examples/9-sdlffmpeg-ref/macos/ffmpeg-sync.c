...
typedef struct AppContext {
    ...
    AVFrame *held_video_frame;   // Holds the *converted* RGB frame if video is early
...
int initialize_decoder(AVCodecContext **dec_ctx_ptr, const AVCodec **codec_ptr, AVStream *stream);
 ...
 * @param video_frame_rgb The video frame to display (converted to RGB24). NULL if no video frame ready now.
 * @param audio_frame_resampled The audio frame to play (resampled). NULL if no audio frame ready now.
 * NOTE: The actual audio data is in ctx->resampled_audio_data.
 * @param video_pts_sec Presentation time of the video frame in seconds. Negative if no video frame.
 * @param audio_pts_sec Presentation time of the audio frame in seconds. Negative if no audio frame.
 */
void playit(AppContext *ctx, AVFrame *video_frame_rgb, AVFrame *audio_frame_resampled, double video_pts_sec, double audio_pts_sec) {
    ...
    printf("playit: ");
    if (video_frame_rgb) {
        // Access RGB data via video_frame_rgb->data[0] and video_frame_rgb->linesize[0]
        // Dimensions: ctx->video_dec_ctx->width, ctx->video_dec_ctx->height
    ...
    if (audio_frame_resampled) {
        // Access resampled audio data via ctx->resampled_audio_data[0], ctx->resampled_audio_data[1]...
        // Format: audio_frame_resampled->format (e.g., AV_SAMPLE_FMT_S16)
        // Sample Rate: audio_frame_resampled->sample_rate
        // Channels: audio_frame_resampled->ch_layout.nb_channels
        // Samples per channel: audio_frame_resampled->nb_samples
        printf("AUD PTS: %8.3f sec (%d samples @ %d Hz)",
               audio_pts_sec,
    ...
    // In a real player, the audio device callback provides a more accurate clock.
    // This simple version sets the clock to the start time of the audio frame being sent.
    if (audio_frame_resampled) {
         ctx->audio_clock = audio_pts_sec;
        //  A slightly better estimation might be end time:
        //  ctx->audio_clock = audio_pts_sec + (double)audio_frame_resampled->nb_samples / audio_frame_resampled->sample_rate;
...
int open_media_file(AppContext *ctx, const char *filename) {
    ...
    if ((ret = avformat_find_stream_info(ctx->fmt_ctx, NULL)) < 0) {
        ...
        // No need to close input here, cleanup() will handle it if fmt_ctx is non-NULL
        return ret;
    }
    ...
    if (ctx->video_stream_idx < 0) {
        ...
        // Allow proceeding without video, maybe? For this example, treat as error.
...
int initialize_decoder(AVCodecContext **dec_ctx_ptr, const AVCodec **codec_ptr, AVStream *stream) {
    ...
    *codec_ptr = codec; // Return found codec if needed elsewhere
    ...
    if ((ret = avcodec_parameters_to_context(codec_ctx, stream->codecpar)) < 0) {
        fprintf(stderr, "ERROR: Failed to copy %s codec parameters to decoder context: %s\n",
                av_get_media_type_string(codec->type), av_err2str(ret));
    ...
    if ((ret = avcodec_open2(codec_ctx, codec, NULL)) < 0) {
        fprintf(stderr, "ERROR: Failed to open %s codec: %s\n",
                av_get_media_type_string(codec->type), av_err2str(ret));
        avcodec_free_context(&codec_ctx); // Free allocated context on error
        return ret;
    }
...
int prepare_decoders_and_conversion(AppContext *ctx) {
    ...
    // --- Initialize Decoders ---
    if ((ret = initialize_decoder(&ctx->video_dec_ctx, &video_codec, video_stream)) < 0) {
        return ret;
    }
    if ((ret = initialize_decoder(&ctx->audio_dec_ctx, &audio_codec, audio_stream)) < 0) {
        return ret;
    }
    ...
    enum AVPixelFormat target_pix_fmt = AV_PIX_FMT_RGB24; // Common display format

    // Allocate buffer for the RGB frame
    // Use av_image_alloc for proper alignment and buffer management
    ret = av_image_alloc(ctx->rgb_frame->data, ctx->rgb_frame->linesize,
                         width, height, target_pix_fmt, 1); // Alignment 1
    if (ret < 0) {
        fprintf(stderr, "ERROR: Could not allocate RGB video buffer: %s\n", av_err2str(ret));
        av_frame_free(&ctx->rgb_frame); // Free the frame struct itself
        return ret;
    }
    // Store the buffer pointer separately for freeing with av_freep(&ctx->rgb_buffer)
    ...
    if (!ctx->sws_ctx) {
        fprintf(stderr, "ERROR: Failed to get SwsContext for video conversion\n");
    ...
    enum AVSampleFormat target_sample_fmt = AV_SAMPLE_FMT_S16; // Signed 16-bit integer
    int target_sample_rate = ctx->audio_dec_ctx->sample_rate; // Or choose a fixed rate like 44100
    // Use the new AVChannelLayout API
    // Get default layout for 2 channels (Stereo)
    ret = av_channel_layout_default(&ctx->target_audio_ch_layout, 2);
     if (ret < 0) {
        fprintf(stderr, "ERROR: Failed to get default stereo channel layout: %s\n", av_err2str(ret));
        return ret; // cleanup() handles freeing other things
    }
    ...
    av_opt_set_chlayout(ctx->swr_ctx, "in_chlayout", &ctx->audio_dec_ctx->ch_layout, 0);
    av_opt_set_int(ctx->swr_ctx, "in_sample_rate", ctx->audio_dec_ctx->sample_rate, 0);
    av_opt_set_sample_fmt(ctx->swr_ctx, "in_sample_fmt", ctx->audio_dec_ctx->sample_fmt, 0);

    av_opt_set_chlayout(ctx->swr_ctx, "out_chlayout", &ctx->target_audio_ch_layout, 0);
    ...
    if ((ret = swr_init(ctx->swr_ctx)) < 0) {
        fprintf(stderr, "ERROR: Failed to initialize the resampling context: %s\n", av_err2str(ret));
    ...
    av_channel_layout_copy(&ctx->resampled_audio_frame->ch_layout, &ctx->target_audio_ch_layout);

    printf("Prepared audio resampling context (%s/%dHz/%s -> %s/%dHz/%s)\n",
        av_get_sample_fmt_name(ctx->audio_dec_ctx->sample_fmt), ctx->audio_dec_ctx->sample_rate, "layout", // TODO: Print input layout nicely
        av_get_sample_fmt_name(target_sample_fmt), target_sample_rate, "stereo"); // Assuming stereo target
...
/**
 * @brief Decodes a packet, processes frames (convert/resample), synchronizes, and calls playit.
...
int decode_and_process_frame(AppContext *ctx, AVPacket *pkt, int flushing) {
    ...
    if (!flushing) {
        ...
        if (ctx->video_dec_ctx) current_dec_ctx = ctx->video_dec_ctx;
        else if (ctx->audio_dec_ctx) current_dec_ctx = ctx->audio_dec_ctx;
        else return 1; // No decoders to flush
        stream_index = (current_dec_ctx == ctx->video_dec_ctx) ? ctx->video_stream_idx : ctx->audio_stream_idx;
    ...
    if (ret < 0 && ret != AVERROR_EOF) { // Ignore EOF when sending NULL packet for flushing
         if (ret == AVERROR(EAGAIN)) {
             // Decoder full, needs receive_frame first. This shouldn't happen often
             // if receive loop below is correct, but handle defensively.
             fprintf(stderr, "WARN: Decoder full (EAGAIN), trying receive first.\n");
             // Fall through to receive loop
         } else {
            fprintf(stderr, "ERROR sending packet for decoding stream %d: %s\n", stream_index, av_err2str(ret));
            return ret;
         }
    }
    ...
    while (1) {
        ...
        if (ret == AVERROR(EAGAIN)) {
            ...
            break; // Exit receive loop, need more packets (or flushing is done for this pkt)
        } else if (ret == AVERROR_EOF) {
            // End of stream for this decoder (flushing complete or end of file)
            av_frame_free(&decoded_frame);
             // If flushing, mark this decoder as done and try the other one next time
             if (flushing) {
                 if (current_dec_ctx == ctx->video_dec_ctx) ctx->video_dec_ctx = NULL;
                 else if (current_dec_ctx == ctx->audio_dec_ctx) ctx->audio_dec_ctx = NULL;
             }
            // If we got EOF after sending packets, break normally.
            // If we got EOF during flushing, signal completion for this decoder.
            got_output = flushing ? 1 : 0; // Signal EOF if flushing
            break; // Exit receive loop
        } else if (ret < 0) {
            fprintf(stderr, "ERROR during decoding stream %d: %s\n", stream_index, av_err2str(ret));
            av_frame_free(&decoded_frame);
        ...
        if (decoded_frame->pts != AV_NOPTS_VALUE) {
            ...
            int64_t best_effort_ts = av_frame_get_best_effort_timestamp(decoded_frame);
             if (best_effort_ts != AV_NOPTS_VALUE) {
                 ...
                 fprintf(stderr, "WARN: Frame (stream %d) lacks PTS, using audio clock %f as fallback.\n", stream_index, pts_sec);
             }
        }
        ...
        if (stream_index == ctx->video_stream_idx) {
            ...
            if (ctx->video_frame_held) {
                // If we are already holding a frame, this new one is probably too early too.
                // A real player would queue frames. Simplification: discard older held frame.
                printf("WARN: Discarding held video frame (PTS %f) to process new one (PTS %f)\n", ctx->held_video_pts_sec, pts_sec);
                 // No need to unref ctx->held_video_frame, it's just a pointer to ctx->rgb_frame
            }
            ...
            if (pts_sec <= ctx->audio_clock || ctx->audio_clock == 0.0) {
                ...
                // Video is too early, hold this frame (its data is already in rgb_frame)
                ctx->held_video_pts_sec = pts_sec;
            ...
            int max_out_samples = (int)av_rescale_rnd(
                swr_get_delay(ctx->swr_ctx, decoded_frame->sample_rate) + decoded_frame->nb_samples,
            ...
            ret = av_samples_alloc_array_and_samples(
                ...
                ctx->resampled_audio_frame->ch_layout.nb_channels, // Number of channels from target frame layout
                max_out_samples,                      // Max samples per channel
                (AVSampleFormat)ctx->resampled_audio_frame->format, // Target format from frame
                0);                                   // Alignment (0 = default)

            if (ret < 0) {
                fprintf(stderr, "ERROR: Could not allocate resampled audio buffer\n");
                // No need to free decoded_frame here, done below
            } else {
                ctx->resampled_audio_buf_size = ret; // Store actual allocated size

                // Perform resampling
                int actual_out_samples = swr_convert(
                    ctx->swr_ctx,
                    ctx->resampled_audio_data, max_out_samples,
                    (const uint8_t **)decoded_frame->data, decoded_frame->nb_samples);

                if (actual_out_samples < 0) {
                    fprintf(stderr, "ERROR while converting audio: %s\n", av_err2str(actual_out_samples));
                    ...
                    // Play the resampled audio (updates audio clock inside playit)
                    playit(ctx, NULL, ctx->resampled_audio_frame, -1.0, pts_sec);

                    // Free the resampled data buffer immediately after playit
                    // (playit should copy the data if it needs it asynchronously)
                    av_freep(&ctx->resampled_audio_data[0]); // Free the buffer allocated by av_samples_alloc...
                    av_freep(&ctx->resampled_audio_data);    // Free the array of pointers
                    ctx->resampled_audio_data = NULL;
                    ctx->resampled_audio_buf_size = 0;

                     // --- Check if held video frame can now be played ---
                    if (ctx->video_frame_held && ctx->held_video_pts_sec <= ctx->audio_clock) {
                         // printf("       Playing held video frame (PTS %f <= clock %f)\n", ctx->held_video_pts_sec, ctx->audio_clock);
                         playit(ctx, ctx->rgb_frame, NULL, ctx->held_video_pts_sec, -1.0);
    ...
    return got_output ? 0 : (flushing ? 1 : 0); // 0 if got output, 1 if flushing finished, 0 otherwise
}
...
void cleanup(AppContext *ctx) {
    ...
    // (held_video_frame just points to rgb_frame's data, no separate free needed)

    // Free conversion/resampling resources
     if (ctx->resampled_audio_data) { // Check if buffer was allocated and maybe not freed in loop
        av_freep(&ctx->resampled_audio_data[0]);
        av_freep(&ctx->resampled_audio_data);
    }
    av_frame_free(&ctx->resampled_audio_frame);
    swr_free(&ctx->swr_ctx);
    av_channel_layout_uninit(&ctx->target_audio_ch_layout); // Uninit layout from av_channel_layout_default

    if (ctx->rgb_buffer) { // Free the buffer allocated by av_image_alloc
         av_freep(&ctx->rgb_buffer); // Use av_freep for buffers from av_malloc/av_image_alloc
...
int main(int argc, char *argv[]) {
    ...
    // decode_and_process_frame handles the NULL pkt logic internally when flushing=1
    while(app_ctx.video_dec_ctx || app_ctx.audio_dec_ctx) { // Loop until both decoders are flushed
