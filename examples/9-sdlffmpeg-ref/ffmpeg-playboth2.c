...
typedef struct {
    ...
    double
        audio_clock;  // Current audio playback time in seconds (master clock)
...
AVFrame *frame_queue_pop(FrameQueue *q, double *pts, int *serial) {
    ...
    while (1) {
        ...
        if (node) {
            ...
            free(node);
            SDL_CondSignal(
                q->cond);  // Signal that space is available (for push)
            break;
        } else if (!q->abort_request) {
            // Wait with a timeout
            int wait_result =
                SDL_CondWaitTimeout(q->cond, q->mutex, QUEUE_POP_TIMEOUT_MS);
    ...
    while (len > 0) {
        if (ctx->audio_buffer_index >= ctx->audio_buffer_size) {
            ...
            int out_samples = av_rescale_rnd(
                swr_get_delay(ctx->swr_ctx, audio_frame->sample_rate) +
                    audio_frame->nb_samples,
                44100, audio_frame->sample_rate, AV_ROUND_UP);

            int out_size = av_samples_get_buffer_size(NULL, 2, out_samples,
                                                      AV_SAMPLE_FMT_S16, 0);
            if (out_size < 0) {
                fprintf(
                    stderr,
                    "audio_callback: av_samples_get_buffer_size() failed\n");
            ...
            int converted =
                swr_convert(ctx->swr_ctx, &ctx->audio_buffer, out_samples,
            ...
            audio_size = converted * 2 *
                         av_get_bytes_per_sample(
                             AV_SAMPLE_FMT_S16);  // 2 channels, S16 format
            ctx->audio_buffer_size =
                audio_size;  // Set buffer size to actual converted data size
            ...
            ctx->audio_clock =
                current_audio_time +
                (double)ctx->audio_buffer_size /
                    (2 * av_get_bytes_per_sample(AV_SAMPLE_FMT_S16) * 44100);
            SDL_UnlockMutex(ctx->audio_clock_mutex);
        }
        ...
        if (to_copy > 0) {
            memcpy(stream, ctx->audio_buffer + ctx->audio_buffer_index,
                   to_copy);
...
static int decode_thread_func(void *arg) {
    ...
    while (!ctx->quit) {
        ...
        if (ret < 0) {
            ...
            // End of stream or error, signal quit after processing remaining
            // packets
            ...
            if (ctx->video_stream_index != -1) {
                ...
                if (null_pkt)
                    packet_queue_push(&video_pkt_queue,
                                      null_pkt);  // NULL packet
            }
            if (ctx->audio_stream_index != -1) {
                AVPacket *null_pkt = av_packet_alloc();
                if (null_pkt)
                    packet_queue_push(&audio_pkt_queue,
                                      null_pkt);  // NULL packet
            }
            break;
        }
        ...
        if (ctx->video_stream_index != -1) {
            ...
            if (packet_queue_pop(&video_pkt_queue, &video_packet, 0, NULL) >
                0) {
                int send_packet_result = avcodec_send_packet(
                    ctx->video_codec_context, &video_packet);
                if (send_packet_result == 0) {
                    // Receive all possible frames from the decoder
                    int receive_frame_result;
                    while (!ctx->quit) {
                        receive_frame_result = avcodec_receive_frame(
                            ctx->video_codec_context, ctx->video_frame);
                        if (receive_frame_result == 0) {
                            // Frame received, convert and push to frame queue
                            AVFrame *rgb_frame_copy = av_frame_alloc();
                            if (!rgb_frame_copy) {
                                fprintf(stderr,
                                        "Decode thread: Error allocating RGB "
                                        "frame copy\n");
                            ...
                            if (av_frame_get_buffer(rgb_frame_copy, 0) < 0) {
                                fprintf(stderr,
                                        "Decode thread: Error allocating "
                                        "buffer for RGB frame copy\n");
                            ...
                            double pts =
                                (ctx->video_frame->pts == AV_NOPTS_VALUE)
                                    ? NAN
                                    : ctx->video_frame->pts *
                                          ctx->video_time_base;

                            frame_queue_push(&ctx->video_queue, rgb_frame_copy,
                                             pts, video_pkt_queue.serial);

                            av_frame_unref(
                                ctx->video_frame);  // Unreference the decoded
                                                    // frame
                        } else if (receive_frame_result == AVERROR(EAGAIN)) {
                            // Decoder needs more input, break receive loop and
                            // send next packet
                            break;
                        } else if (receive_frame_result == AVERROR_EOF) {
                            // End of stream for this decoder, break receive
                            // loop
                            break;
                        } else {
                            fprintf(stderr,
                                    "Decode thread: Error receiving video "
                                    "frame: %s\n",
                    ...
                    fprintf(stderr,
                            "Decode thread: Error sending video packet: %s\n",
        ...
        if (ctx->audio_stream_index != -1) {
            ...
            if (packet_queue_pop(&audio_pkt_queue, &audio_packet, 0, NULL) >
                0) {
                int send_packet_result = avcodec_send_packet(
                    ctx->audio_codec_context, &audio_packet);
                if (send_packet_result == 0) {
                    // Receive all possible frames from the decoder
                    int receive_frame_result;
                    while (!ctx->quit) {
                        receive_frame_result = avcodec_receive_frame(
                            ctx->audio_codec_context, ctx->audio_frame);
                        if (receive_frame_result == 0) {
                            // Frame received, push to audio frame queue
                            AVFrame *audio_frame_copy = av_frame_alloc();
                            if (!audio_frame_copy) {
                                fprintf(stderr,
                                        "Decode thread: Error allocating audio "
                                        "frame copy\n");
                            ...
                            av_frame_move_ref(
                                audio_frame_copy,
                                ctx->audio_frame);  // Move reference

                            double pts =
                                (ctx->audio_frame->pts == AV_NOPTS_VALUE)
                                    ? NAN
                                    : ctx->audio_frame->pts *
                                          ctx->audio_time_base;

                            frame_queue_push(&ctx->audio_queue,
                                             audio_frame_copy, pts,
                                             audio_pkt_queue.serial);

                            av_frame_unref(
                                ctx->audio_frame);  // Unreference the decoded
                                                    // frame
                        } else if (receive_frame_result == AVERROR(EAGAIN)) {
                            // Decoder needs more input, break receive loop and
                            // send next packet
                            ...
                            fprintf(stderr,
                                    "Decode thread: Error receiving audio "
                                    "frame: %s\n",
                                    av_err2str(receive_frame_result));
                            ctx->quit = 1;  // Error, signal quit
                            break;
                    ...
                    fprintf(stderr,
                            "Decode thread: Error sending audio packet: %s\n",
                            av_err2str(send_packet_result));
                    ctx->quit = 1;  // Error, signal quit
                }
...
int init_media_player(MediaContext *ctx, const char *filepath) {
    ...
    for (int i = 0; i < ctx->format_context->nb_streams; i++) {
        if (ctx->format_context->streams[i]->codecpar->codec_type ==
            AVMEDIA_TYPE_VIDEO) {
            if (ctx->video_stream_index == -1) ctx->video_stream_index = i;
        } else if (ctx->format_context->streams[i]->codecpar->codec_type ==
                   AVMEDIA_TYPE_AUDIO) {
            if (ctx->audio_stream_index == -1) ctx->audio_stream_index = i;
        }
    }
    ...
    if (ctx->video_stream_index != -1) {
        ...
        // Get video time base
        ctx->video_time_base = av_q2d(
            ctx->format_context->streams[ctx->video_stream_index]->time_base);
        ...
        // Create texture
        ctx->texture = SDL_CreateTexture(ctx->renderer, SDL_PIXELFORMAT_RGB24,
    ...
    if (ctx->audio_stream_index != -1) {
        ...
        av_opt_set_int(ctx->swr_ctx, "out_sample_rate", 44100,
                       0);  // Target sample rate
        av_opt_set_sample_fmt(ctx->swr_ctx, "in_sample_fmt",
                              audio_codecpar->format, 0);
        av_opt_set_sample_fmt(ctx->swr_ctx, "out_sample_fmt", AV_SAMPLE_FMT_S16,
                              0);  // Target sample format
        ...
        // Get audio time base
        ctx->audio_time_base = av_q2d(
            ctx->format_context->streams[ctx->audio_stream_index]->time_base);

        // Initialize SDL for audio
        // Note: SDL_Init(SDL_INIT_AUDIO) might have already been called if
        // video exists. SDL_Init is safe to call multiple times with the same
        // flags.
    ...
    // subsystems This block might be redundant if SDL_Init is called separately
    // for video and audio, but it ensures both are initialized if both streams
    // are found.
    if (ctx->video_stream_index != -1 && ctx->audio_stream_index != -1) {
        if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO |
                     SDL_INIT_GAMECONTROLLER) < 0) {
            fprintf(stderr,
                    "SDL could not initialize video and audio! SDL_Error: %s\n",
                    SDL_GetError());
            return -1;  // Cleanup will be handled by stop_media_player
        }
    }
...
int play_media(MediaContext *ctx) {
    ...
    if (ctx->video_stream_index != -1) {
        current_video_frame = frame_queue_pop(
            &ctx->video_queue, &current_video_pts, &current_video_serial);
    ...
    while (!ctx->quit) {
        while (SDL_PollEvent(&e)) {
            if (e.type == SDL_QUIT) {
                ...
                if (e.window.event == SDL_WINDOWEVENT_SIZE_CHANGED) {
                    // Handle window resize if needed (e.g., update renderer
                    // viewport) SDL_RenderSetViewport(ctx->renderer, NULL); //
        ...
        if (ctx->video_stream_index != -1) {
            if (current_video_frame) {
                ...
                // Simple synchronization: drop frame if video is too far behind
                // audio
                // If video is more than 100ms behind audio
                if (!isnan(av_diff) && av_diff < -0.1) {
                    fprintf(stderr,
                            "Main loop: Video behind audio (diff: %.3f), "
                            "dropping frame.\n",
                            av_diff);
                    av_frame_free(&current_video_frame);
                    current_video_frame =
                        frame_queue_pop(&ctx->video_queue, &current_video_pts,
                                        &current_video_serial);
                    if (!current_video_frame) {
                    ...
                    // Recalculate frame_delay based on next frame (basic
                    // estimation)
                    AVFrame *next_frame =
                        frame_queue_peek(&ctx->video_queue, NULL, NULL);
                    if (next_frame && !isnan(current_video_pts)) {
                        ...
                        if (!isnan(next_pts)) {
                            ...
                            frame_delay =
                                0.04;  // Default if next PTS is unknown
                        }
                    } else {
                ...
                if (time_diff <= 0) {
                    ...
                    // Recalculate frame_delay based on the new current frame
                    // and the next frame (basic estimation)
                    AVFrame *next_frame =
                        frame_queue_peek(&ctx->video_queue, NULL, NULL);
                    if (next_frame && !isnan(current_video_pts)) {
                        double next_pts = NAN;
                ...
                current_video_frame =
                    frame_queue_pop(&ctx->video_queue, &current_video_pts,
                                    &current_video_serial);
                if (!current_video_frame) {
                    // Still no frame or timeout, check quit flag and wait
                    // briefly
                    if (ctx->quit) break;
                    SDL_Delay(1);
                } else {
