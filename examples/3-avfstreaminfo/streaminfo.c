...
int print_avformat_stream_info() {
    ...
    if (util_get_first_media_file(path_buffer, 1024) < 0) {
        return -1;
    }
    ...
    /* per api docs, avformat_open_input shouldn't need this, but CEMU crashes
     * w/o */
        ...
        if (stream) {
            ...
            if (codec_par) {
                ...
                if (codec_par->codec_type == AVMEDIA_TYPE_VIDEO) {
                    WHBLogPrintf("  Resolution: %d x %d", codec_par->width,
                                 codec_par->height);
                } else if (codec_par->codec_type == AVMEDIA_TYPE_AUDIO) {
                    WHBLogPrintf("  Sample rate: %d Hz",
